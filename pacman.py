...
def readCommand(argv):
    ...
    parser.add_option(
        '--maxfoods',
        dest='maxfoods',
        help=default('Comma separated values to fixes max number of small and big foods. Applies only when rshuffle is activated.'),
        default="1,0")
    ...
    if options.rshuffle:
        ...
        indices_foods = random.sample(
            set(indices), min(
                nb_foods, len(indices)))
        indices_bigfoods = random.sample(set(
            indices) - set(indices_foods), min(nb_bigfoods, len(set(indices) - set(indices_foods))))
...
def loadAgentCustom(module_name, nographics):
